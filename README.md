...
> MediaFire's `direct_download` (actually-playable) link type works on **every**
> account, free included — it's not paid-gated. Free accounts share a **50 GB/day**
> bandwidth pool for it; once that's used up for the day, further streams fall back to
> a non-playable view-page link until the pool resets. A paid MediaFire account keeps
> streaming working past that daily cap. `/download-url` always resolves a
> `normal_download` link instead (no bandwidth-pool dependency), so listeners can save
> files offline regardless of the streaming pool's state.
...
`supabase/migration_mediafire_only.sql` after `migration_labels_albums.sql`.
