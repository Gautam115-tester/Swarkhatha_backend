...
// Returns the best playable URL we can get. Tries direct_download first
// (paid accounts); falls back to the normal view-page URL otherwise.
async function getStreamLink({ sessionToken, quickKey }) {
  try {
    ...
    if (link) return { url: link, direct: true };
  ...
  return { url: normalLink, direct: false };
}
...
module.exports = { getSessionToken, getAccountInfo, uploadFile, getStreamLink, deleteFile };
