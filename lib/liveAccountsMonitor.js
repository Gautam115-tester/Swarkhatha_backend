 ...
 * no MediaFire traffic; the moment the first one connects the poll
 * starts, and it stops the moment the last one disconnects.
 *
...
class LiveAccountsMonitor extends EventEmitter {
  ...
  /**
   * Reference-counted start/stop: the 8s MediaFire poll should only run
   ...
   * still share the single underlying timer/MediaFire calls.
   */
  acquire() {
    this._listenerCount++;
