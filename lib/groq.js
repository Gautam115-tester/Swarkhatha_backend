...
// of transliterateToHinglish() so it can be retried on a smaller slice
// (see below) without duplicating the request-building/parsing logic.
...
// Retries a batch that failed with Groq's JSON-generation error by
// splitting it in half and trying each half independently -- a smaller
// batch means a smaller expected output, which is the actual lever that
// fixes a truncation-caused failure (unlike blindly retrying the same
// batch again, which would just as likely truncate the same way). Bottoms
// out at single-line batches; if even one line can't come back as valid
// JSON, that's a real error worth surfacing, not a sizing problem.
async function transliterateBatchWithRetry({ apiKey, batch }) {
  try {
    return await callHinglishBatch({ apiKey, batch });
  } catch (e) {
    if (isJsonGenerationFailure(e) && batch.length > 1) {
      const mid = Math.ceil(batch.length / 2);
      const firstHalf = await transliterateBatchWithRetry({ apiKey, batch: batch.slice(0, mid) });
      const secondHalf = await transliterateBatchWithRetry({ apiKey, batch: batch.slice(mid) });
...
async function transliterateToHinglish({ apiKey, segments }) {
  ...
  for (let i = 0; i < segments.length; i += SEGMENTS_PER_BATCH) {
    ...
    const lines = await transliterateBatchWithRetry({ apiKey, batch });
