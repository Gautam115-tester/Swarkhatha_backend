...
// One media item in, one label name out (or null if genuinely nothing
// fits) -- picks from EXACTLY the admin-managed content_labels list
// passed in, never invents a new one. Same request shape as
// callHinglishBatch (json_object response_format, same TEXT_MODEL/
// keepAliveClient/authHeaders), just a much shorter prompt/response.
//
// Deliberately conservative: the prompt explicitly allows "null" when
// nothing is a good fit, because a wrong category is worse than no
// category (it actively misleads browse/filter), and the caller
// (autoCategorizeIfMissing in lib/autoCategorize.js) only ever fills a
// category that's currently missing -- an admin's own explicit choice is
// never overwritten by this.
async function suggestCategory({ apiKey, title, artistOrNarrator, albumOrSeries, type, labelNames }) {
  if (!labelNames || labelNames.length === 0) return null;

  const kind = type === 'music' ? 'a song' : 'an audio story episode';
  const context = [
    `Title: ${title || '(untitled)'}`,
    artistOrNarrator ? `Artist/Narrator: ${artistOrNarrator}` : null,
    albumOrSeries ? `Album/Series: ${albumOrSeries}` : null
  ].filter(Boolean).join('\n');

  const prompt = `You are categorizing ${kind} for a listening app, using ONLY the app's existing category list -- never invent a new one.
...
Reply with strict JSON: {"category": "<exact name from the list above>"} if one genuinely fits, or {"category": null} if none of them do. Do not guess if you are not reasonably confident -- null is a correct, useful answer, a wrong category is not.`;
  ...
  try {
    ...
    throw new Error(`Groq returned non-JSON for a category suggestion: ${e.message}`);
  }

  const chosen = parsed.category;
  if (!chosen || typeof chosen !== 'string') return null;
  // Only trust an EXACT match against the real list -- if Groq
  // paraphrased/hallucinated a close-but-not-quite name, that's the same
  // as no confident answer, not a reason to invent a new category.
  return labelNames.includes(chosen) ? chosen : null;
}

module.exports = { validateKey, transcribe, transliterateToHinglish, suggestCategory, mimeForFileName, extractErrorMessage, parseRetryAfterMs };