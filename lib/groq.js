...
// True for errors worth retrying once: Groq-side 5xx responses and
...
async function transcribe({ apiKey, buffer, fileName, mime }) {
  ...
  try {
    ...
    // One retry for a transient (5xx / network) failure only -- Groq's
    // Whisper endpoint occasionally 500s on a perfectly fine file, and a
    // second attempt with a fresh multipart body usually goes through.
    // A 4xx (bad key, bad file, unsupported format) is retried as-is and
    // will just fail again, so it's raised immediately instead.
    if (!isTransient(e)) throw e;
    await sleep(2000);
    const retryForm = new FormData();
    retryForm.append('file', buffer, { filename: fileName || 'audio.mp3', contentType: mime || 'audio/mpeg' });
    ...
    resp = await keepAliveClient.post(`${BASE}/audio/transcriptions`, retryForm, {
      headers: { ...authHeaders(apiKey), ...retryForm.getHeaders() },
      maxBodyLength: Infinity,
      maxContentLength: Infinity
    });
  }
